...
function TabDelegados({ comision, idModelo }) {
  ...
  async function handleAsignar() {
    if (!idSel) return;
    setError('');
    try { await asignarDelegado(comision.id, { idParticipante: Number(idSel) }); setIdSel(''); cargar(); }
    catch (err) { setError(err.message); }
  }

  async function handleRemover(idP) {
    try { await removerDelegado(comision.id, idP); cargar(); }
    catch (err) { setError(err.message); }
  }

  const asignadosIds = new Set(delegados.map(d => d.idParticipante));
  const disponibles  = participantes.filter(p => !asignadosIds.has(p.id));
  ...
  return (
    <div>
      <div className="d-flex gap-2 mb-3">
        ...
        <button className="btn btn-primary btn-sm flex-shrink-0" onClick={handleAsignar}>Asignar</button>
      </div>
      {error && <div className="alert alert-danger py-1 small mb-2">{error}</div>}

      <div className="table-responsive">
        <table className="table table-dark table-sm table-hover">
          <thead><tr><th>#</th><th>Delegado</th><th>Representación</th><th>Estado</th><th></th></tr></thead>
          <tbody>
            {delegados.map(d => (
            ...
            {delegados.length === 0 && (
              <tr><td colSpan={5} className="text-secondary text-center">Sin delegados asignados.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default function GestorComisiones() {
  ...
  const [subTab, setSubTab]             = useState('mesa'); // 'mesa' | 'delegados'
  ...
  async function cargarComisiones(id) {
    try { setComisiones(await getComisiones(id)); }
                    ...
                    <div className="d-flex justify-content-between align-items-start">
                      <div>
                        ...
                        <div className="text-secondary" style={{ fontSize: '0.75rem' }}>
                          {c.nosRegistros}/{c.maxParticipantes} delegados
                        </div>
                      </div>
          ...
          {comisionSel && (
            <div className="card bg-dark border-secondary">
                ...
                <div className="ms-auto d-flex gap-2">
                  {['mesa', 'delegados'].map(t => (
                    <button key={t} className={`btn btn-sm ${subTab === t ? 'btn-primary' : 'btn-outline-secondary'}`}
                      onClick={() => setSubTab(t)}>
                      {t === 'mesa' ? 'Mesa Directiva' : 'Delegados'}
                    </button>
                  ))}
                </div>
              </div>
              <div className="card-body">
                ...
                {subTab === 'delegados' && <TabDelegados comision={comisionSel} idModelo={idModelo} />}
              </div>
            </div>
          )}
        </>
