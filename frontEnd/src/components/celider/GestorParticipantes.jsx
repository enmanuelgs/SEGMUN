...
const VACIO = { nombres: '', apellidos: '', representacion: '', numeracionPLERD: '',
                numeroTelefono: '', correo: '', regional: '', distrito: '', llaveUnica: '' };
...
export default function GestorParticipantes() {
  ...
  function handleModeloChange(id) { setIdModelo(id); cargar(id); }

  function iniciarEdicion(p) {
    setEditando(p.id);
    setForm({ nombres: p.nombres, apellidos: p.apellidos, representacion: p.representacion ?? '',
              numeracionPLERD: p.numeracionPLERD ?? '', numeroTelefono: p.numeroTelefono ?? '',
              correo: p.correo ?? '', regional: p.regional ?? '', distrito: p.distrito ?? '',
  ...
  const campo = (key, placeholder, requerido = false) => (
        ...
        <>
          <form onSubmit={handleGuardar} className="card bg-dark border-secondary p-3 mb-4">
            ...
            <div className="row g-2">
              ...
              <div className="col-md-3">{campo('representacion', 'Representación (país)')}</div>
              <div className="col-md-3">{campo('numeracionPLERD', 'Numeración PLERD')}</div>
              ...
              <div className="col-md-2">{campo('distrito', 'Distrito')}</div>
          ...
          <div className="table-responsive">
            <table className="table table-dark table-hover table-sm">
              <thead>
                <tr><th>PLERD</th><th>Apellidos</th><th>Nombres</th><th>Representación</th><th>Teléfono</th><th>Correo</th><th></th></tr>
              </thead>
              <tbody>
                {filtrados.map(p => (
                  <tr key={p.id}>
                    <td className="text-secondary small">{p.numeracionPLERD ?? '—'}</td>
                    <td>{p.apellidos}</td>
                    <td>{p.nombres}</td>
                    <td>{p.representacion ?? '—'}</td>
                ...
                {filtrados.length === 0 && (
                  <tr><td colSpan={7} className="text-secondary text-center">
